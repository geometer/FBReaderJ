...
final class ZLTreeResource extends ZLResource {
	...
	public static void loadData(String language) {
		...
		try {
			reader.readDocument(ourRoot, ZLResourceFile.createResourceFile("resources/zlibrary/" + fileName).getInputStream());
		} catch (IOException e) {
		}
		try {
			reader.readDocument(ourRoot, ZLResourceFile.createResourceFile("resources/application/" + fileName).getInputStream());
		} catch (IOException e) {
		}
	}
	...
	private static class ResourceTreeReader extends ZLXMLReaderAdapter {
		...
		public void readDocument(ZLTreeResource root, InputStream stream) {
			myStack.clear();
			myStack.add(root);
			read(stream);
		}
