	...
	private Response serveCover(String uri, Method method, Map<String,String> headers, Map<String,String> params, Map<String,String> files) {
		try {
			...
			if (image instanceof ZLFileImageProxy) {
				...
				final Response res = NanoHTTPD.newChunkedResponse(Response.Status.OK, MimeType.IMAGE_PNG.toString(), stream);
				...
				if (pluginImage.isSynchronized()) {
					try {
						...
						final Response res = NanoHTTPD.newChunkedResponse(Response.Status.OK, MimeType.IMAGE_JPEG.toString(), is);
	...
	private Response serveFile(ZLFile file, String mime, Map<String,String> headers) throws IOException {
		...
		if (range == null || !range.startsWith(BYTES_PREFIX)) {
			if (etag.equals(headers.get("if-none-match")))
				res = NanoHTTPD.newFixedLengthResponse(Response.Status.NOT_MODIFIED, mime, "");
			else {
				res = NanoHTTPD.newChunkedResponse(Response.Status.OK, mime, baseStream);
			...
			if (start >= fileLength) {
				...
				res = NanoHTTPD.newChunkedResponse(
