...
class XHTMLTagAsideAction : public XHTMLGlobalTagAction {
...
void XHTMLTagAsideAction::doAtStart(XHTMLReader &reader, const char** xmlattributes) {
	if (!reader.myNewParagraphInProgress) {
		reader.pushTextKind(REGULAR);
		reader.beginParagraph();
		reader.myNewParagraphInProgress = true;
	}
}

void XHTMLTagAsideAction::doAtEnd(XHTMLReader &reader) {
	endParagraph(reader);
...
void XHTMLReader::fillTagTable() {
	if (ourTagActions.empty()) {
		...
		addAction("aside", new XHTMLTagAsideAction());
...
void XHTMLReader::startElementHandler(const char *tag, const char **attributes) {
	...
	std::string sId; //should be in the same scope as id
	if (sTag == "aside") {
		if (id == 0 || id[0] == '\0') {
			std::ostringstream idStream;
			idStream << "FBReaderJ-autogenerated-" << myModelReader.asideCount();
			sId = idStream.str();
			id = sId.c_str();
		}
		myModelReader.addHyperlinkControl(FOOTNOTE, myReferenceAlias + HASH + id);
		static const std::string STAR = "*";
		myModelReader.addData(STAR);
		...
		if (pEpubType != 0 && id[0] !='\0') {
			const std::string sEpubType = ZLUnicodeUtil::toLower(pEpubType);
			if (sEpubType == "footnote" || sEpubType == "rearnote") {
				myModelReader.setPopupTextModel(myReferenceAlias + HASH + id);
...
void XHTMLReader::endElementHandler(const char *tag) {
	...
	if (myTagDataStack.empty() || !myTagDataStack.back()->InFootnote) {
		myModelReader.unsetPopupTextModel();
	}
}
